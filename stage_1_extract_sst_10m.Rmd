...
```{r}
  hex=read.csv("./records data/records_cleaned_harm_23_01_26.csv") %>% 
  select(h3_id) %>% 
  distinct()
```
...
chunk_size <- 1 # Adust this value based on  RAM and internet speed
...
Crop data exactly to the hexagons borders
Loop it
```{r}
...
test=read_rds("copernicus_data_by_hexagon/MEDSEA_MULTIYEAR_PHY_006_004_10m/831e9afffffffff.rds")  %>%
     ...
     unite(
         ...
         long,
         sep = "_",
         remove = FALSE  # Set to TRUE if want to remove the original lat/long columns
     ) %>% 
...
#[1] "831ec9fffffffff" "832d05fffffffff"
# These are the black and Marmara Sea which are outside the scope of this work
```
...
# Adjust the function (e.g., dmy, mdy) if  date format is different.
data <- data %>%
...
mean_sst_df <- data %>%
  ...
  p <- ggplot(data = mean_sst_df, aes(x = date, y = mean_sst_spatial)) +
    geom_line(alpha = 0.5) + # Add points with some transparency
    geom_smooth(method = "lm",formula = y~x,color="black")+
    scale_x_date(date_breaks = "4 years", date_labels = "%Y") + # Set x-axis breaks to 4-year intervals
    labs(
      title = paste("SST Trend for Hexagon:", hex_name),
      x = "Date",
      y = "Mean SST (°C)"
    ) +
    theme_bw() +
