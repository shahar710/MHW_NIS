...
Bind climatologies and MHWs with thinned records data
...
```{r}
#detrended
clim_dt=read_rds("./climatology_mhws/climatology_dt_df_2.rds")
events_dt=read_rds("./climatology_mhws/event_dt_df_2.rds")

#Records
records=read_csv("./records data/records_cleaned_harm_23_01_26.csv") %>% 
...
#2) detrended baseline cumulative intensity
...
#write_rds(annual_c_int_dt,"./climatology_mhws/annual_c_int_dt.rds")
```
...
filtered_data_before_first_record_dt <- final_data_clean_dt %>%
  ...
  group_by(h3_id,species) %>%
  filter(sum(records)>0) %>% #remoce time series with no records
  ungroup()
...
#write.csv(annual_sst_lm_slopes,"./copernicus_data_by_hexagon/MEDSEA_MULTIYEAR_PHY_006_004_10m//corrected_hex_level_data/final_data/annual_sst_rates.csv",row.names = F)
...
ormef <- read.delim("./records data/ORMEF/89476.tsv") %>% 
  mutate(
...
Add (optional) the predicted oremf sources within the data points after 2020 - however colinearity issues arise when using this.
```{r}
library(glmmTMB)
#####################################
...
```{r}
year_of_f_rec=read.csv("./records data/records_cleaned_harm_23_01_26.csv") %>% 
  group_by(species) %>% 
...
#            "./records data/record_data_for_glmms.rds")
```
...
# 2. Convert  'records' data frame to an sf object
...
# 3. Calculate the extent of coordinates
...
rec_thinned_map=ggplot() +
  ...
  # Crop the map to the extent of points
  coord_sf(
...
# ggsave(filename ="./plots/rec_thinned_map.png" ,plot =rec_thinned_map ,device = "jpeg",units = "cm",width = 25,height = 12)
```
...
records_map <- ggplot() +
  ...
  labs(
    title = "A) First hexagon-level records of non-indigenous Red Sea species (1987-2023)",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_bw() +
...
region_annots <- data.frame(
  ...
  y     = c(190, 190, 190)   # Set this to the top of Y-axis
)
...
ggsave(filename ="./plots/intro_grid2_23_01_26.png" ,plot =intro_grid ,device = "jpeg",units = "cm",width = 20,height = 20)
```
