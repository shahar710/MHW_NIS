...
```{r eval=FALSE, include=FALSE}
test=sst_data %>% 
         filter(h3_id%in%"831e12fffffffff")
...
ggplot() + 
  geom_point(data=test,
             aes(x=date,y=median_sst_10m))+
  geom_line(data=detrend_data,
            aes(x=date,y=fitted))
#plot detrended data
ggplot() + 
  geom_point(data=detrend_data,
...
```{r}
test_climatology_dt = heatwaveR::ts2clm(
  data = sst_data_detrended %>% 
    filter(h3_id %in% "830004fffffffff"),
  x = date,
  y = residuals,
  c("1993-01-01", "2021-06-30"),
  windowHalfWidth = 5,
  pctile = 95,
  smoothPercentile = T,
  clmOnly = F,
  var = F,
  roundClm = 4) %>% 
  mutate(hexagon="830004fffffffff")
...
write_rds(ts2clm_det_df,"ts2clm_dt_df_median_sst_10m.rds")
```
...
Write detect_event output
```{r eval=FALSE, include=FALSE}
write_rds(climatology_dt_df,"climatology_dt_df.rds")
write_rds(event_dt_df,"event_dt_df.rds")
```